 ...
 */
function getPhone() {
    var areaCode = Math.floor(Math.random() * (999 - 200 + 1) + 200);
    var firstThreeDigits = Math.floor(Math.random() * 1000);
    var lastFourDigits = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
    return "+1 (".concat(areaCode, ") ").concat(firstThreeDigits, "-").concat(lastFourDigits);
}
